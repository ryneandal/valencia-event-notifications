...
- **Style**: formatting / linters (`ruff format`, `ruff check`) and type usage (Pydantic/typing).
...
Provide Copilot these files or paths as context in prompts so it understands the repo layout (the project uses a `src/` layout):
...
- Formatting & linting: **ruff** (`uv run ruff format` and `uv run ruff check`). Add pre-commit hooks to enforce.  
